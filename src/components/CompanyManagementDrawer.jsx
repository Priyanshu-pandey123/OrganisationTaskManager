    ...
    <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
      ...
      <div className="md:col-span-1">
        ...
        <select
          ...
          className="w-full px-2 py-2 bg-white dark:bg-gray-600 text-gray-900 dark:text-gray-200 
          rounded-lg border border-gray-300 dark:border-gray-500 
          focus:outline-none focus:ring-2 focus:ring-indigo-500"
        >
          <option value="">Select Team</option>
          {currentCompanyTeams.map((team, index) => (
            <option key={team.id || index} value={team.team_name}>
              {team.team_name}
            </option>
          ))}
        </select>
      ...
      <div className="md:col-span-2">
        ...
        <input
          ...
          className="w-full px-4 py-2 bg-white dark:bg-gray-600 text-gray-900 dark:text-gray-200 
          rounded-lg border border-gray-300 dark:border-gray-500 
          focus:outline-none focus:ring-2 focus:ring-indigo-500"
        />
      </div>
      ...
      <div className="md:col-span-1 flex items-end">
        <button
          ...
          className="w-full px-6 py-2 bg-indigo-600 hover:bg-indigo-700 
          disabled:bg-gray-400 disabled:cursor-not-allowed 
          text-white font-medium rounded-lg transition duration-200 
          flex items-center justify-center space-x-2 h-[42px]"
        >
          {invitingMember ? (
            <>
              <svg className="w-4 h-4 animate-spin" fill="none" viewBox="0 0 24 24">
          ...
          {currentCompanyTeams.length > 0 && (
            ...
            {currentCompanyTeams.map((team, index) => (
              <div
                key={team.id || index}
                className="bg-white dark:bg-gray-600 rounded-lg p-4 border border-gray-200 dark:border-gray-600"
              >
                {/* LEFT + RIGHT SPLIT */}
                <div className="flex items-center justify-between w-full">
          
                  {/* LEFT SIDE: Icon + Team Name */}
                  <div className="flex items-center space-x-3">
                    <div className="w-8 h-8 bg-green-500 rounded-lg flex items-center justify-center">
                      <svg className="w-4 h-4 text-white" fill="none" viewBox="0 0 24 24" stroke="currentColor">
